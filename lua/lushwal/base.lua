...
local scheme = lush(function()
	...
	return {
		...
		Visual({ bg = colors.br_black, fg = colors.background }),
		...
		ColorColumn({ bg = colors.subtle10 }),
		CursorColumn({ fg = colors.subtle_magneta }),
		CursorLine({ bg = colors.subtle10, gui = "None" }),
		CursorLineNr({ fg = colors.white, bg = colors.background, gui = "None" }),
		PMenu({ fg = colors.white, bg = colors.subtle_yellow }),
		...
		DiffChange({ fg = colors.br_grey, bg = colors.subtle10 }),
