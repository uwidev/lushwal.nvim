...
local scheme = lush(function()
	...
	return {
		...
		Folded({ fg = colors.white, bg = colors.subtle_magenta, gui = "italic" }),
		IncSearch({ fg = colors.br_black, bg = colors.harsh_green }),

		VertSplit({ fg = colors.white, bg = colors.background }),
		WinSeparator({ VertSplit }),

		Macro({ fg = colors.red }),
		MatchParen({ fg = colors.foreground, bg = colors.harsh_red }),
		ModeMsg({ fg = colors.green }),
		MoreMsg({ fg = colors.green }),
		Question({ fg = colors.blue }),
		Search({ fg = colors.br_grey, bg = colors.harsh_yellow }),
		SpecialKey({ fg = colors.br_grey }),
		TooLong({ fg = colors.red }),
		Underlined({ fg = colors.red }),
		Visual({ bg = colors.harsh_magenta, fg = colors.background }),
		...
		ColorColumn({ bg = colors.subtle25 }),
		CursorColumn({ fg = colors.subtle_magneta }),
		CursorLine({ bg = colors.subtle25, gui = "None" }),
		CursorLineNr({ fg = colors.white, bg = colors.background, gui = "None" }),
		PMenu({ fg = colors.white, bg = colors.subtle_cyan }),
		...
		DiffAdd({ fg = colors.green, bg = colors.subtle_green, gui = "bold" }),
		DiffChange({ fg = colors.harsh_yellow, bg = colors.subtle10 }),
		DiffDelete({ fg = colors.red, bg = colors.subtle_red, gui = "bold" }),
		DiffText({ fg = colors.blue, bg = colors.subtle_blue }),
